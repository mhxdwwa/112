...
function showJianghuResult(overlay, won, student, pet, investCoins, boss) {
  const scene = overlay.querySelector('#jhScene');
  let coinResult = 0;
  let coinDelta = 0; // v119: Track actual net coin change for accurate log snapshot
  let growthGain = 0;
  if (won) {
    ...
    coinDelta = coinResult - investCoins; // v119: Net change = +2x (not 3x)
    growthGain = 0; // 胜利不再获得成长值
  } else {
    // 失败：失去投入的金币，获得投入金币30%的成长值
    coinResult = -investCoins;
    coinDelta = -investCoins; // v119: Net change = -investCoins
  ...
  // Record action
  // v119: Use coinDelta (actual net change) instead of coinResult (gross reward)
