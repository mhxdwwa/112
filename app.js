...
function applyAction(student, action, pet){ let coinsChange = action.coins; let isPenalty = coinsChange < 0; let expChange = 0; let prevGrowth = pet.growth; if(pet.isDead){ if(isPenalty){ showNotification('操作禁止','宠物已死亡，不能施加惩罚','error'); return false; } else { student.coins += coinsChange; if(student.coins < 0) student.coins = 0; if(pet.penaltyStreak !== undefined) pet.penaltyStreak = 0; recordAction(student.id, student.name, '奖惩', `${action.name} (宠物死亡)`, coinsChange, 0, pet.id); showNotification(action.name, `+${coinsChange}金币 (宠物死亡无法获得经验)`, 'success'); if(window.USE_API&&window.ApiMigration){window.ApiMigration.coinsAndPet(student,coinsChange,[{petId:pet.id,updates:{penalty_streak:0}}],{actionType:'奖惩',details:action.name+' (宠物死亡)',expDelta:0,petId:pet.id}).then(function(r){if(r.ok)student.coins=r.coinsAfter;});} return true; } } if(isPenalty){ let absDeduct = Math.abs(coinsChange); let coinDeducted = Math.min(absDeduct, student.coins); let remaining = absDeduct - coinDeducted; student.coins -= coinDeducted; expChange = 0; if(remaining > 0){ expChange = -remaining; pet.growth += expChange; if(pet.growth <= 0){ pet.growth = 0; pet.isDead = true; pet.deathGrowth = prevGrowth; pet.deathDate = new Date().toISOString(); pet.penaltyStreak = 0; recordAction(student.id, student.name, '惩罚致死', `${action.name} 导致死亡（金币不足，经验扣至0）`, -absDeduct, -prevGrowth, pet.id, {causedDeath: true, prevGrowth: prevGrowth}); showNotification('惩罚致死',`${pet.nickname||pet.name} 金币不足，经验被扣至0，宠物死亡！`,'error'); saveClassData('coins');if(window.USE_API&&window.ApiMigration){window.ApiMigration.coinsAndPet(student,coinsChange,[{petId:pet.id,updates:{growth:0,is_dead:true,penalty_streak:0}}],{actionType:'惩罚致死',details:action.name,expDelta:expChange,petId:pet.id,checkBalance:true}).then(function(r){if(r.ok)student.coins=r.coinsAfter;});} if(typeof renderPKPage==="function") renderPKPage(); return true; } updatePetLevel(student, pet.id, expChange); } let msg = `${action.name}：金币-${coinDeducted}`; if(expChange !== 0) msg += `，经验${expChange}`; recordAction(student.id, student.name, '奖惩', msg, -coinDeducted, expChange, pet.id); showNotification(action.name, msg, 'warning'); } else { if(pet.penaltyStreak !== undefined) pet.penaltyStreak = 0; student.coins += coinsChange; if(student.coins < 0) student.coins = 0; let msg = `${action.name}：金币+${coinsChange}`; recordAction(student.id, student.name, '奖惩', msg, coinsChange, 0, pet.id); showNotification(action.name, msg, 'success'); }
  // v141: API 模式 — 同步金币和宠物变更到 Supabase
  if (window.USE_API && window.ApiMigration) {
    var _apiPetUpdates = [{ petId: pet.id, updates: { growth: pet.growth, penalty_streak: pet.penaltyStreak || 0 } }];
    ...
    cur.students.forEach(function(s) {
      if (selectedIds.includes(s.id.toString())) {
        ...
        if (_pet && (isCustom || (!isCustom && !isCustomXd))) {
          window.ApiMigration.coinsAndPet(s, action.coins, [{
            petId: _pet.id,
            ...
            expDelta: 0, petId: _pet.id, checkBalance: action.coins < 0
          }).then(function(r) { if(r.ok) s.coins = r.coinsAfter; });
        } else if (isCustom && !_pet) {
          window.ApiMigration.changeStudentCoins(s, action.coins, '批量奖惩', action.name, 0, null)
            .then(function(r) { if(r.ok) s.coins = r.coinsAfter; });
        }
      }
    });
