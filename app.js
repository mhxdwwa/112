...
function deleteClass(id){if(confirm('确定删除该班级？删除后可在"已删除班级"中恢复')){const cls=classesData.find(c=>c.id===id||c.id==id);var clsName=cls?cls.name:'';var clsIdForSync=cls?cls.id:id;if(cls){const snapshot={id:cls.id,name:cls.name,students:JSON.parse(JSON.stringify(cls.students)),deletedAt:new Date().toISOString()};deletedClasses.push(snapshot);if(deletedClasses.length>20)deletedClasses.shift();saveDeletedClasses();}classesData=classesData.filter(c=>c.id!==id&&c.id!=id);if(currentClassId===id||currentClassId==id)currentClassId=classesData[0]?.id||null;if(typeof customActions!=='undefined'){customActions=customActions.filter(a=>a.class_id!=id);}if(typeof _userDeletedClassIds!=='undefined'&&_userDeletedClassIds.indexOf(clsIdForSync)===-1){_userDeletedClassIds.push(clsIdForSync);}saveClassData();renderClassList();scheduleAllRenders();showNotification('班级已删除','可在"已删除班级"中恢复','info');if(typeof db!=='undefined'&&db&&typeof currentUser!=='undefined'&&currentUser){(async()=>{try{if(typeof _isValidInt4Id==='function'&&_isValidInt4Id(clsIdForSync)){await db.from('classes').delete().eq('id',clsIdForSync).eq('teacher_id',currentUser.id);}else if(clsName){var r=await db.from('classes').select('id').eq('teacher_id',currentUser.id).eq('name',clsName).limit(1);if(r.data&&r.data.length>0){var sid=r.data[0].id;var stuR=await db.from('students').select('id').eq('class_id',sid);var sids=(stuR.data||[]).map(function(s){return s.id;});if(sids.length>0){await db.from('pets').delete().in('student_id',sids);await db.from('students').delete().in('id',sids);}await db.from('custom_actions').delete().eq('class_id',sid);await db.from('classes').delete().eq('id',sid);}}console.log('[DAL] deleteClass: Supabase cleanup OK for',clsName||id);}catch(e){console.warn('[DAL] deleteClass: Supabase cleanup failed:',e.message);}})();}}}
