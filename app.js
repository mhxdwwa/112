...
function deleteClass(id){if(confirm('确定删除该班级？删除后可在"已删除班级"中恢复')){const cls=classesData.find(c=>c.id===id||c.id==id);var clsName=cls?cls.name:'';var clsIdForSync=cls?cls.id:id;if(cls){const snapshot={id:cls.id,name:cls.name,students:JSON.parse(JSON.stringify(cls.students)),deletedAt:new Date().toISOString()};deletedClasses.push(snapshot);if(deletedClasses.length>20)deletedClasses.shift();saveDeletedClasses();}classesData=classesData.filter(c=>c.id!==id&&c.id!=id);if(currentClassId===id||currentClassId==id)currentClassId=classesData[0]?.id||null;if(typeof customActions!=='undefined'){customActions=customActions.filter(a=>a.class_id!=id);}saveClassData();renderClassList();scheduleAllRenders();showNotification('班级已删除','可在"已删除班级"中恢复','info');if(typeof db!=='undefined'&&db&&typeof currentUser!=='undefined'&&currentUser){(async()=>{try{var targetClassId=clsIdForSync;if(typeof _isValidInt4Id==='function'&&_isValidInt4Id(clsIdForSync)){targetClassId=clsIdForSync;}else if(clsName){var r=await db.from('classes').select('id').eq('teacher_id',currentUser.id).eq('name',clsName).limit(1);if(r.data&&r.data.length>0){targetClassId=r.data[0].id;}else{console.warn('[DAL] deleteClass: class not found by name:',clsName);return;}}else{return;}var stuR=await db.from('students').select('id').eq('class_id',targetClassId);var sids=(stuR.data||[]).map(function(s){return s.id;});if(sids.length>0){var petDel=await db.from('pets').delete().in('student_id',sids);if(petDel.error)console.warn('[DAL] deleteClass: pet delete error:',petDel.error);var stuDel=await db.from('students').delete().in('id',sids);if(stuDel.error)console.warn('[DAL] deleteClass: student delete error:',stuDel.error);}var caDel=await db.from('custom_actions').delete().eq('class_id',targetClassId);if(caDel.error)console.warn('[DAL] deleteClass: custom_actions delete error:',caDel.error);var clsDel=await db.from('classes').delete().eq('id',targetClassId).eq('teacher_id',currentUser.id);if(clsDel.error)console.warn('[DAL] deleteClass: class delete error:',clsDel.error);console.log('[DAL] deleteClass: Supabase cleanup OK for',clsName||id,'(deleted',sids.length,'students)');}catch(e){console.warn('[DAL] deleteClass: Supabase cleanup failed:',e.message);}})();}}}
...
function deleteStudentById(id){if(confirm('删除学生')){const cur=classesData.find(c=>c.id===currentClassId);const stu=cur.students.find(s=>s.id.toString()===id.toString());var stuIdForDelete=stu?stu.id:null;cur.students=cur.students.filter(s=>s.id.toString()!==id.toString());saveClassData();closeModal();showStudentListModal();scheduleAllRenders();if(currentModalStudentId && currentModalStudentId===id) closeModal();if(typeof db!=='undefined'&&db&&stuIdForDelete&&stuIdForDelete>0){(async()=>{try{var petDel=await db.from('pets').delete().eq('student_id',stuIdForDelete);if(petDel.error)console.warn('[DAL] deleteStudent: pet delete error:',petDel.error);var stuDel=await db.from('students').delete().eq('id',stuIdForDelete);if(stuDel.error)console.warn('[DAL] deleteStudent: student delete error:',stuDel.error);console.log('[DAL] deleteStudent: Supabase cleanup OK for student',stuIdForDelete);}catch(e){console.warn('[DAL] deleteStudent: cleanup failed:',e.message);}})();}}}
function clearAllStudents(){if(confirm('清空所有学生？')){const cur=classesData.find(c=>c.id===currentClassId);var studentIds=cur.students.map(function(s){return s.id;}).filter(function(id){return id>0;});cur.students=[];saveClassData();closeModal();scheduleAllRenders();if(currentModalStudentId) closeModal();if(typeof db!=='undefined'&&db&&studentIds.length>0){(async()=>{try{var petDel=await db.from('pets').delete().in('student_id',studentIds);if(petDel.error)console.warn('[DAL] clearStudents: pet delete error:',petDel.error);var stuDel=await db.from('students').delete().in('id',studentIds);if(stuDel.error)console.warn('[DAL] clearStudents: student delete error:',stuDel.error);console.log('[DAL] clearStudents: Supabase cleanup OK (deleted',studentIds.length,'students)');}catch(e){console.warn('[DAL] clearStudents: cleanup failed:',e.message);}})();}}}
