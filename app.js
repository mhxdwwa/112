...
function restoreClass(id){var numId=parseInt(id);var idx=deletedClasses.findIndex(c=>c.id==numId||c.id==id);if(idx===-1){showNotification('恢复失败','未找到该班级数据','error');return;}const cls=deletedClasses[idx];const restored={id:Date.now().toString(),name:cls.name,students:JSON.parse(JSON.stringify(cls.students))};restored.students.forEach(function(stu){stu.id=Date.now()+Math.random()*1000|0;stu.pets.forEach(function(pet){pet.id=Date.now()+Math.random()*1000|0;});});classesData.push(restored);deletedClasses.splice(idx,1);saveClassData();saveDeletedClasses();currentClassId=restored.id;renderClassList();scheduleAllRenders();showNotification('恢复成功',`班级【${cls.name}】已恢复`,'success');}
