...
function scheduleAllRenders() {
  ...
  // v214: 仅当 PK/江湖行页面可见时才调度重渲染，避免隐藏页面的无效 DOM 操作
  // 切换页面时 switchPage() 会主动触发对应渲染，不依赖此处调度
  var pkPage = document.getElementById('pk-page');
  var jhPage = document.getElementById('jianghu-page');
  var flags = 0;
  if (pkPage && pkPage.classList.contains('active')) flags |= _RF_PK;
  if (jhPage && jhPage.classList.contains('active')) flags |= _RF_JH;
  if (flags) scheduleRender(flags);
}
...
// v214: localStorage 备份防抖定时器 — IndexedDB 已就绪时，localStorage 仅作降级备份，无需每次同步写入
var _lsBackupTimers = {};
function safeLSSave(key, data) {
  ...
  if (typeof StorageDB !== 'undefined' && StorageDB.isReady()) {
    ...
    // v214: IndexedDB 已就绪时，localStorage 备份延迟 500ms 防抖执行
    // 快速连续操作（如批量奖惩、全班喂食）只触发一次 localStorage 写入
    // 而非每次都同步 JSON.stringify + localStorage.setItem 阻塞主线程
    if (_lsBackupTimers[key]) clearTimeout(_lsBackupTimers[key]);
    _lsBackupTimers[key] = setTimeout(function() {
      _lsBackupTimers[key] = null;
      _safeLSSaveSync(key, data);
    }, 500);
    return;
  }
  // IndexedDB 未就绪，直接同步写入 localStorage
  _safeLSSaveSync(key, data);
}
// localStorage 同步写入（含降级策略）
function _safeLSSaveSync(key, data) {
  try {
...
function changeStudentCoins(student, delta, actionType, details, expDelta, petId, extra) {
  ...
  if (window.USE_API && window.ApiMigration) {
    ...
    // v169: 返回 Promise 以便调用方可以 await
    var apiPromise = window.ApiMigration.changeStudentCoins(student, delta, actionType, details, expDelta, petId, extra)
...
var _optimisticLogSaveTimer = null;
function _recordOptimisticLog(studentId, studentName, actionType, details, coinDelta, expDelta, petId) {
  // v213: removed early return for (0,0) — 仙丹-only operations have coinDelta=0 and expDelta=0 but still need history
  var log = {
    id: _genLocalId(), timestamp: new Date().toISOString(),
  ...
  // v182: Update local log timestamp tracker
  if (log.timestamp > _lastLocalLogTime) _lastLocalLogTime = log.timestamp;
  // v214: 防抖持久化到 IndexedDB（异步，不阻塞主线程）
  // 替代原来的同步 localStorage.setItem(JSON.stringify(...))，消除主线程阻塞
  if (_optimisticLogSaveTimer) clearTimeout(_optimisticLogSaveTimer);
  _optimisticLogSaveTimer = setTimeout(function() {
    _optimisticLogSaveTimer = null;
    if (typeof StorageDB !== 'undefined' && StorageDB.isReady()) {
      StorageDB.save('operationLogs', window.operationLogs);
    } else {
...
function restoreToLogEntry(logId){
  ...
  if(window.USE_API&&window.ApiMigration){
    ...
    if(window.ApiMigration.saveQuizState){
      window.ApiMigration.saveQuizState(student.id, null, student.quizState ? JSON.stringify(student.quizState) : null).then(function(r){
        if(!r.ok){ _restoreErrors.push('游戏进度'); console.warn('[v210] restoreQuizState error:', r.error); }
      }).catch(function(e){ _restoreErrors.push('游戏进度'); console.warn('[v210] restoreQuizState failed:', e); });
    }
    ...
    if(window.ApiMigration.changeStudentCoins && snap.coinsAfter !== undefined && _restoreCoinDelta !== 0){
      window.ApiMigration.changeStudentCoins(student, _restoreCoinDelta, '恢复数据', '恢复到 '+log.timestamp, 0, pet ? pet.id : null).then(function(r){
        if(!r.ok){ _restoreErrors.push('金币'); console.warn('[v210] restoreCoins error:', r.error); }
      }).catch(function(e){ _restoreErrors.push('金币'); console.warn('[v210] restoreCoins failed:', e); });
    }
    if(pet && window.ApiMigration.updatePet){
      window.ApiMigration.updatePet(pet.id, {growth: pet.growth, level: pet.level, is_dead: pet.isDead||false}).then(function(r){
        if(!r.ok){ _restoreErrors.push('宠物'); console.warn('[v210] restorePet error:', r.error); }
      }).catch(function(e){ _restoreErrors.push('宠物'); console.warn('[v210] restorePet failed:', e); });
    }
    // 延迟检查错误并通知用户
    setTimeout(function(){
      if(_restoreErrors.length > 0){
        showNotification('恢复部分失败', _restoreErrors.join('、') + ' 同步失败，请重试', 'warning');
      }
    }, 3000);
  }
...
function revertToLog(logId){
  ...
  if(log.fullSnapshot){
    ...
    if(window.USE_API&&window.ApiMigration){
      ...
      curClass.students.forEach(function(stu){
        if(window.ApiMigration.saveQuizState){
          window.ApiMigration.saveQuizState(stu.id, null, stu.quizState ? JSON.stringify(stu.quizState) : null).then(function(r){
            if(!r.ok){ _revertErrors.push(stu.name); console.warn('[v210] revertQuizState error for', stu.name, ':', r.error); }
          }).catch(function(e){ _revertErrors.push(stu.name); console.warn('[v210] revertQuizState failed for', stu.name, ':', e); });
        }
      });
      // 延迟检查错误并通知用户
      setTimeout(function(){
        if(_revertErrors.length > 0){
          showNotification('撤销部分失败', _revertErrors.join('、') + ' 同步失败，请重试', 'warning');
        }
      }, 3000);
    }
    scheduleAllRenders();
  ...
  if(log.extra && log.extra.pkType && log.extra.pkType !== 'draw'){
    ...
    log.reverted = true;
    // v223: 用时间戳匹配配对日志（ID 基于 Date.now()*1000，差值远大于 5）
    var logTime = new Date(log.timestamp).getTime();
    const pairLog = _logs.find(function(l) {
      if (String(l.id) === String(logId)) return false;
      if (!l.extra || !l.extra.pkType || !l.extra.opponentId) return false;
      if (l.extra.opponentId.toString() !== log.studentId.toString()) return false;
      // 时间戳在 10 秒内视为同一次 PK
      var lTime = new Date(l.timestamp).getTime();
      return Math.abs(lTime - logTime) < 10000;
    });
  ...
  if(log.extra && log.extra.pkType === 'draw'){
    log.reverted = true;
    // v223: 用时间戳匹配配对日志
    var drawLogTime = new Date(log.timestamp).getTime();
    const pairLog = _logs.find(function(l) {
      if (String(l.id) === String(logId)) return false;
      if (!l.extra || l.extra.pkType !== 'draw' || !l.extra.opponentId) return false;
      if (l.extra.opponentId.toString() !== log.studentId.toString()) return false;
      var lTime = new Date(l.timestamp).getTime();
      return Math.abs(lTime - drawLogTime) < 10000;
    });
    if(pairLog) pairLog.reverted = true;
    saveLogs();
    // v187: 强制刷新历史弹窗（撤销后按钮状态需更新）
...
function classDailyCheckin(){ if(typeof currentUser!=='undefined'&&currentUser&&currentUser.type==='student'){showNotification('权限不足','此操作仅限教师','error');return;} if(!currentClassId){showNotification('请先选择班级','请在左侧选择一个班级后再打卡','warning');return;} if(checkPauseAndNotify())return; const cur=classesData.find(c=>c.id===currentClassId); if(!cur){showNotification('班级数据异常','未找到当前班级数据','error');return;} if(!cur.students||cur.students.length===0){showNotification('暂无学生','请先添加学生','warning');return;} let checkedCount=0; let skipNoPet=0; const _checkinItems=[]; const _checkinStudents=[]; cur.students.forEach(s=>{if(!s.pets||s.pets.length===0){skipNoPet++;return;} if(_hasCheckedInToday(s)){return;} s.lastCheckinDate=new Date().toISOString(); checkedCount++; _checkinStudents.push(s); if(window.USE_API&&window.ApiMigration){ s.coins += 10; _checkinItems.push({studentId:s.id,studentName:s.name,coinDelta:10,actionType:'全班打卡',details:'+10金币',expDelta:0,petId:null,petUpdates:[],checkBalance:false}); }else{ var p=changeStudentCoins(s, 10, '全班打卡', '+10金币', 0, null); } }); if(checkedCount===0){let reason='全班同学今天都已经打过卡了'; if(skipNoPet>0) reason+=`（${skipNoPet}人未领养宠物，不参与打卡）`; showNotification('今日已打卡',reason,'info');return;} if(window.USE_API&&window.ApiMigration&&_checkinItems.length>0){ _checkinStudents.forEach(function(s){window.ApiMigration.updateStudent(s.id,{last_checkin_date:s.lastCheckinDate}).catch(function(e){console.warn('[v210] updateStudent checkin error for',s.name,':',e);});}); window.ApiMigration.batchCoins(_checkinItems).then(function(result){ if(result.ok){ (result.results||[]).forEach(function(r){if(r.ok){var stu=cur.students.find(function(s){return s.id===r.studentId;});if(stu)stu.coins=r.coinsAfter;}}); saveClassData('coins'); }else{ console.error('[v179] Checkin batch error:',result.error); _checkinStudents.forEach(function(s){s.coins=Math.max(0,(s.coins||0)-10);s.lastCheckinDate=null;}); saveClassData('coins'); scheduleAllRenders(); } }).catch(function(e){ console.warn('[v212] classDailyCheckin batchCoins failed:', e); _checkinStudents.forEach(function(s){s.coins=Math.max(0,(s.coins||0)-10);s.lastCheckinDate=null;}); saveClassData('coins'); scheduleAllRenders(); }); } saveClassData('coins'); scheduleAllRenders(); if(typeof refreshHistoryModalIfOpen==='function')refreshHistoryModalIfOpen(true); if(currentModalStudentId) refreshCurrentStudentModal(); let msg=`${checkedCount}人打卡成功，每人+10金币`; if(skipNoPet>0) msg+=`（${skipNoPet}人未领养宠物，已跳过）`; showNotification('全班打卡',msg,'success'); }
function classAllFeed(){ if(typeof currentUser!=='undefined'&&currentUser&&currentUser.type==='student'){showNotification('权限不足','此操作仅限教师','error');return;} if(!currentClassId){showNotification('请先选择班级','请在左侧选择一个班级后再喂食','warning');return;} if(checkPauseAndNotify())return; const cur=classesData.find(c=>c.id===currentClassId); if(!cur){showNotification('班级数据异常','未找到当前班级数据','error');return;} if(!cur.students||cur.students.length===0){showNotification('暂无学生','请先添加学生','warning');return;} let fedCount=0,skipDead=0,skipCoins=0,skipMax=0,skipNoPet=0,skipFed=0; const upgrades=[]; const _feedBatchItems=[]; cur.students.forEach(s=>{const pet=getGrowablePet(s); if(!pet && (!s.pets||s.pets.length===0)){skipNoPet++;return;} if(!pet && s.pets.every(p=>p.level>=9)){skipMax++;return;} if(!pet && s.pets.every(p=>p.isDead)){skipDead++;return;} if(!pet){skipMax++;return;} if(_hasFedToday(pet)){skipFed++;return;} if(s.coins<5){skipCoins++;return;} let gain=2; pet.growth+=gain; pet.lastFeedDate=new Date().toISOString(); const upResult=updatePetLevel(s, pet.id, gain, true); if(upResult) upgrades.push(upResult); if(window.USE_API&&window.ApiMigration){s.coins -= 5; _feedBatchItems.push({studentId:s.id,studentName:s.name,coinDelta:-5,actionType:'全班喂食',details:pet.nickname||pet.name+' +'+gain+'成长值',expDelta:gain,petId:pet.id,petUpdates:[{petId:pet.id,updates:{growth:pet.growth,last_feed_date:pet.lastFeedDate,level:pet.level}}],checkBalance:false}); _recordOptimisticLog(s.id, s.name, '全班喂食', pet.nickname||pet.name+' +'+gain+'成长值', -5, gain, pet.id);}else{changeStudentCoins(s, -5, '全班喂食', `${pet.nickname||pet.name} +${gain}成长值`, gain, pet.id);} fedCount++;}); if(fedCount===0){let reason=''; if(skipFed>0)reason+=`${skipFed}人今天已喂食 `; if(skipDead>0)reason+=`${skipDead}人宠物已死亡 `; if(skipCoins>0)reason+=`${skipCoins}人金币不足 `; if(skipMax>0)reason+=`${skipMax}人全部满级 `; if(skipNoPet>0)reason+=`${skipNoPet}人未领养宠物`; showNotification('无法喂食',reason||'没有可喂食的宠物','info');return;} if(window.USE_API&&window.ApiMigration&&_feedBatchItems.length>0){ window.ApiMigration.batchCoins(_feedBatchItems).then(function(result){ if(result.ok){ (result.results||[]).forEach(function(r){if(r.ok){var stu=cur.students.find(function(s){return s.id===r.studentId;});if(stu)stu.coins=r.coinsAfter;}}); saveClassData('pet'); }else{ console.error('[v179] Feed batch error:',result.error); _feedBatchItems.forEach(function(item){var stu=cur.students.find(function(s){return s.id===item.studentId;});if(stu){stu.coins=(stu.coins||0)+5;var pet=getGrowablePet(stu);if(pet){pet.growth=Math.max(0,(pet.growth||0)-2);_recalcPetLevel(pet);pet.lastFeedDate=null;}}}); saveClassData('pet'); scheduleAllRenders(); } }).catch(function(e){ console.warn('[v212] classAllFeed batchCoins failed:', e); _feedBatchItems.forEach(function(item){var stu=cur.students.find(function(s){return s.id===item.studentId;});if(stu){stu.coins=(stu.coins||0)+5;var pet=getGrowablePet(stu);if(pet){pet.growth=Math.max(0,(pet.growth||0)-2);_recalcPetLevel(pet);pet.lastFeedDate=null;}}}); saveClassData('pet'); scheduleAllRenders(); }); } saveClassData('pet'); scheduleAllRenders(); if(typeof refreshHistoryModalIfOpen==='function')refreshHistoryModalIfOpen(true); if(currentModalStudentId) refreshCurrentStudentModal(); let msg=`${fedCount}只宠物喂食成功，每只+2成长值，-5金币`; if(skipFed+skipDead+skipCoins+skipMax+skipNoPet>0){let skips=[]; if(skipFed>0)skips.push(`${skipFed}人今天已喂食`); if(skipDead>0)skips.push(`${skipDead}人宠物已死亡`); if(skipCoins>0)skips.push(`${skipCoins}人金币不足`); if(skipMax>0)skips.push(`${skipMax}人全部满级`); if(skipNoPet>0)skips.push(`${skipNoPet}人未领养宠物`); msg+=`（跳过：${skips.join('、')}）`;} showNotification('全班喂食',msg,'success'); showBatchUpgradeNotice(upgrades); }
...
function confirmDeletePet(studentId, petId){
  ...
  if(wasActive){
    ...
    if (window.USE_API && window.ApiMigration) {
      window.ApiMigration.updateStudent(student.id, { active_pet_id: student.activePetId }).catch(function(e){ console.warn('[v212] updateStudent active_pet_id failed:', e); });
    }
  }
  // Save data
  saveClassData('pet');
...
function _deletePetFromSupabase(petId, studentId){
  ...
  if (window.USE_API && window.ApiMigration) {
    window.ApiMigration.deletePet(petId, studentId).then(function(result) {
      if (result.ok) {
...
function _generateStudentCardHTML(s){
  ...
  const inlineTitleHtml = fx.titleHtml ? fx.titleHtml.replace('shop-title-badge','shop-title-badge inline-title') : '';
  // 学生名字固定3字位：2字名中间加空格，1字名前加两空格
  const rawStudentName = s.name;
...
function renderHomePetGrid(){ const grid=document.getElementById('homePetGrid');
  ...
  grid.querySelectorAll('.home-pet-card[data-sid]').forEach(function(card) {
    ...
    existingOrder.push(card.dataset.sid);
  });
  var newSids = {};
  var newOrder = [];
  _gridStudents.forEach(function(s) { newSids[s.id] = true; newOrder.push(String(s.id)); });
  // 检测排序是否变化（排序按钮点击后顺序会变，但卡片数据hash不变）
  var orderChanged = existingOrder.length !== newOrder.length;
  if (!orderChanged) {
    for (var _i = 0; _i < existingOrder.length; _i++) {
      if (existingOrder[_i] !== newOrder[_i]) { orderChanged = true; break; }
    }
  }
  // 移除不再存在的学生卡片
...
function playWithPet(student,pet){
  ...
  if (window.USE_API && window.ApiMigration) {
    ...
    var prevCoins = student.coins;
    var prevLevel = pet.level; // v203: 保存回滚用等级
    var prevLastPlayDate = pet.lastPlayDate; // v220: 保存回滚用玩耍日期
    ...
    window.ApiMigration.coinsAndPet(student, -20, [{
      ...
      else { pet.growth = prevGrowth; pet.level = prevLevel; pet.lastPlayDate = prevLastPlayDate; student.coins = prevCoins; } // v220: 恢复原始玩耍日期
    });
...
function applyAction(student, action, pet){ let coinsChange = action.coins; let isPenalty = coinsChange < 0; let expChange = 0; let prevGrowth = pet.growth; if(pet.isDead){ if(isPenalty){ showNotification('操作禁止','宠物已死亡，不能施加惩罚','error'); return false; } else { student.coins += coinsChange; if(student.coins < 0) student.coins = 0; if(pet.penaltyStreak !== undefined) pet.penaltyStreak = 0; if(!(window.USE_API&&window.ApiMigration)){recordAction(student.id, student.name, '奖惩', `${action.name} (宠物死亡)`, coinsChange, 0, pet.id);} showNotification(action.name, `+${coinsChange}金币 (宠物死亡无法获得经验)`, 'success'); if(window.USE_API&&window.ApiMigration){window.ApiMigration.coinsAndPet(student,coinsChange,[{petId:pet.id,updates:{penalty_streak:0}}],{actionType:'奖惩',details:action.name+' (宠物死亡)',expDelta:0,petId:pet.id}).then(function(r){if(r.ok)student.coins=r.coinsAfter;}).catch(function(e){console.warn('[v212] coinsAndPet petDeath failed:', e);}); _recordOptimisticLog(student.id, student.name, '奖惩', action.name+' (宠物死亡)', coinsChange, 0, pet.id);} return true; } } if(isPenalty){ let absDeduct = Math.abs(coinsChange); let coinDeducted = Math.min(absDeduct, student.coins); let remaining = absDeduct - coinDeducted; student.coins -= coinDeducted; expChange = 0; if(remaining > 0){ expChange = -remaining; pet.growth += expChange; if(pet.growth <= 0){ pet.growth = 0; pet.isDead = true; pet.deathGrowth = prevGrowth; pet.deathDate = new Date().toISOString(); pet.penaltyStreak = 0; if(!(window.USE_API&&window.ApiMigration)){recordAction(student.id, student.name, '惩罚致死', `${action.name} 导致死亡（金币不足，经验扣至0）`, -absDeduct, -prevGrowth, pet.id, {causedDeath: true, prevGrowth: prevGrowth});} showNotification('惩罚致死',`${pet.nickname||pet.name} 金币不足，经验被扣至0，宠物死亡！`,'error'); saveClassData('coins');if(window.USE_API&&window.ApiMigration){window.ApiMigration.coinsAndPet(student,-coinDeducted,[{petId:pet.id,updates:{growth:0,is_dead:true,penalty_streak:0}}],{actionType:'惩罚致死',details:action.name,expDelta:expChange,petId:pet.id,checkBalance:true}).then(function(r){if(r.ok)student.coins=r.coinsAfter;}).catch(function(e){console.warn('[v212] coinsAndPet penaltyDeath failed:', e);}); _recordOptimisticLog(student.id, student.name, '惩罚致死', action.name+' 导致死亡', -coinDeducted, expChange, pet.id);} if(typeof renderPKPage==="function") renderPKPage(); return true; } updatePetLevel(student, pet.id, expChange); } let msg = `${action.name}：金币-${coinDeducted}`; if(expChange !== 0) msg += `，经验${expChange}`; if(!(window.USE_API&&window.ApiMigration)){recordAction(student.id, student.name, '奖惩', msg, -coinDeducted, expChange, pet.id);} showNotification(action.name, msg, 'warning'); } else { if(pet.penaltyStreak !== undefined) pet.penaltyStreak = 0; student.coins += coinsChange; if(student.coins < 0) student.coins = 0; let msg = `${action.name}：金币+${coinsChange}`; if(!(window.USE_API&&window.ApiMigration)){recordAction(student.id, student.name, '奖惩', msg, coinsChange, 0, pet.id);} showNotification(action.name, msg, 'success'); }
  ...
  if (window.USE_API && window.ApiMigration) {
    ...
    if (pet.isDead) { _apiPetUpdates[0].updates.is_dead = true; }
    ...
    window.ApiMigration.coinsAndPet(student, coinsChange, _apiPetUpdates, {
      ...
      console.warn('[v212] coinsAndPet applyAction failed:', e);
    });
...
function modalDailyCheckin(){
  ...
  if(window.USE_API&&window.ApiMigration){
    window.ApiMigration.updateStudent(student.id, { last_checkin_date: student.lastCheckinDate }).catch(function(e){ console.warn('[v212] updateStudent checkin date failed:', e); });
  }
  saveClassData();
...
function confirmChangePet(studentId){var rawNick=document.getElementById('nicknameInput')?.value||selectedPetName;var nickname=_sanitizeInput(rawNick)||selectedPetName;if(nickname.length>5)nickname=nickname.slice(0,5);const cur=classesData.find(c=>c.id===currentClassId);const student=cur.students.find(s=>s.id.toString()===studentId.toString());const activePet=getActivePet(student);if(!activePet||activePet.level>=2)return;const idx=student.pets.findIndex(p=>p.id===activePet.id);if(idx!==-1){student.pets[idx]={...activePet,name:selectedPetName,nickname:nickname,level:1,growth:activePet.growth,lastFeedDate:activePet.lastFeedDate||new Date().toISOString(),todayFeedCount:0,isDead:activePet.isDead, todayPlayCount:0, lastPlayDate:null, penaltyStreak:activePet.penaltyStreak||0};saveClassData('pet');if(window.USE_API&&window.ApiMigration){window.ApiMigration.updatePet(student.pets[idx].id,{name:selectedPetName,nickname:nickname,level:1,growth:activePet.growth,is_dead:activePet.isDead}).catch(function(e){console.warn('[v212] updatePet changePet failed:', e);});}closeModal();scheduleAllRenders();showNotification('更换成功',`新宠物${nickname}`,'success');}}
function renamePet(studentId, petId){ const cur=classesData.find(c=>c.id===currentClassId); if(!cur) return; const student=cur.students.find(s=>s.id.toString()===studentId.toString()); if(!student) return; const pet=student.pets.find(p=>String(p.id)===String(petId)); if(!pet) return; const oldName=pet.nickname||pet.name; showModal('修改宠物名字',`<div style="text-align:center;margin-bottom:10px;font-size:14px;color:#888;">当前名字：<strong>${esc(oldName)}</strong></div><div style="text-align:center;margin-bottom:8px;font-size:12px;color:#aaa;">最多5个字</div><input id="renamePetInput" value="${escAttr(oldName)}" maxlength="5" style="width:100%;padding:10px 14px;border:2px solid #ffcfcf;border-radius:16px;font-size:16px;text-align:center;outline:none;box-sizing:border-box;" onfocus="this.select()" placeholder="请输入新名字">`,[{text:'取消',onclick:'closeModal()'},{text:'确认修改',onclick:`confirmRenamePet('${escJS(studentId)}','${escJS(petId)}')`}]); setTimeout(()=>{const inp=document.getElementById('renamePetInput');if(inp)inp.focus();},100); }
function confirmRenamePet(studentId, petId){ const inp=document.getElementById('renamePetInput'); if(!inp) return; var rawName=inp.value.trim(); if(!rawName){showNotification('名字不能为空','请输入一个宠物名字','warning');return;} var v=_validateInput(rawName,5,'宠物昵称'); if(!v.ok){showNotification('输入无效',v.error,'warning');return;} const cur=classesData.find(c=>c.id===currentClassId); if(!cur) return; const student=cur.students.find(s=>s.id.toString()===studentId.toString()); if(!student) return; const pet=student.pets.find(p=>String(p.id)===String(petId)); if(!pet) return; const oldName=pet.nickname||pet.name; pet.nickname=v.value; saveClassData('pet');if(window.USE_API&&window.ApiMigration){window.ApiMigration.updatePet(pet.id,{nickname:v.value}).catch(function(e){console.warn('[v212] updatePet rename failed:', e);});} closeModal(); scheduleAllRenders(); if(currentModalStudentId && student.id.toString()===currentModalStudentId.toString()) refreshCurrentStudentModal(); showNotification('改名成功',`${oldName} → ${v.value}`,'success'); }
...
function setActivePet(student, petId){ petId = Number(petId); if(student.pets.some(p=>Number(p.id)===petId)){ student.activePetId = petId; saveClassData('pet');if(window.USE_API&&window.ApiMigration){window.ApiMigration.updatePet(petId,{is_active:true}).catch(function(e){console.warn('[v212] updatePet setActive failed:', e);});student.pets.forEach(function(p){if(Number(p.id)!==petId){window.ApiMigration.updatePet(p.id,{is_active:false}).catch(function(e){console.warn('[v212] updatePet deactivate failed:', e);});}});} renderHomePetGrid(); if(typeof renderPKPage==="function") renderPKPage(); if(currentModalStudentId && student.id.toString()===currentModalStudentId.toString()) refreshCurrentStudentModal(); } }
    ...
    if (isCustomXd && xdUpdatedCount > 0) {
      cur.students.forEach(function(s) {
        if (selectedIds.includes(s.id.toString())) {
          ...
          var _details = '自定义' + _sign + customXdVal + '仙丹，共' + (s.xiandan || 0) + '枚';
          var _pet = getActivePet(s);
          // 1. 本地乐观日志
