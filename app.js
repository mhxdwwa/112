...
function showUpgradeEffect(petName, level, petConfigId, petNickname, oldLevel, studentName) {
  ...
  const oldImgSrc = `images/${petConfigId}/${oldLevel}.png`;
  const newImgSrc = `images/${petConfigId}/${level}.png`;
...
// ========== 全自动新宠物发现机制 ==========
// 无需手动配置！系统会自动尝试加载 images/数字/1.png
...
function recordAction(studentId, studentName, actionType, details, coinDelta, expDelta, petId, extra = null){
  ...
  const log = {
    id: Date.now() + Math.random(), timestamp: new Date().toISOString(),
...
function updatePetLevel(student, petId, growthDelta=0, silent=false) { const pet = student.pets.find(p=>p.id===petId); if(!pet) return false; if(pet.level >= 9) return false; const cfg = PET_CONFIG[pet.name]; if(!cfg) return false; let oldLevel = pet.level; let newLevel = pet.level; for(let i=cfg.stages.length-1;i>=0;i--) if(pet.growth>=cfg.stages[i].growthRequired){ newLevel=cfg.stages[i].stage; break; } if(newLevel > oldLevel){ pet.level = newLevel; if(newLevel >= 9){ const maxGrowth = cfg.stages[cfg.stages.length-1].growthRequired; pet.growth = Math.min(pet.growth, maxGrowth); } const stageName = cfg.stages[newLevel-1]?.stageName||`阶段${newLevel}`; if(!silent){ showUpgradeEffect(pet.name, newLevel, cfg.id, pet.nickname||pet.name, oldLevel, student.name); showNotification('宠物升级',`🎉 恭喜 ${student.name} 同学的 ${pet.nickname||pet.name} 升到${stageName}！`,'success'); scheduleAllRenders(); if(currentModalStudentId && student.id.toString()===currentModalStudentId.toString()) refreshCurrentStudentModal(); } return {studentName:student.name, petName:pet.nickname||pet.name, petRealName:pet.name, newLevel:newLevel, oldLevel:oldLevel, stageName:stageName, cfgId:cfg.id}; } return false; }
...
function editCustomAction(id){const act=customActions.find(a=>a.id===id);if(!act)return;closeModal();showModal('编辑奖惩',`<input id="editName" value="${esc(act.name)}"><input id="editCoins" value="${act.coins}" type="number">`,[{text:'取消',onclick:'showCustomActionsModal()'},{text:'保存',onclick:`updateReward('${id}')`}]);}
function updateReward(id){const name=document.getElementById('editName')?.value.trim();const coins=parseInt(document.getElementById('editCoins')?.value);if(name&&!isNaN(coins)){const idx=customActions.findIndex(a=>a.id===id);if(idx!==-1){customActions[idx]={...customActions[idx],name,coins};saveCustomActions();}closeModal();showCustomActionsModal();}}
function deleteCustomAction(id){if(confirm('删除操作')){customActions=customActions.filter(a=>a.id!==id);saveCustomActions();showCustomActionsModal();}}
...
function getRandomSkill(){ return PK_SKILLS[Math.floor(Math.random()*PK_SKILLS.length)]; }
...
async function startPKBattleLoop(student1, student2, p1, p2) {
  ...
  while(p1HP > 0 && p2HP > 0 && turn < 5) {
    ...
    if(!r1.dodged && r1.dmgDealt > firstTargetMaxHP * 0.22 && Math.random() < 0.40) {
      await sleep(600);
    ...
    if(!r2.dodged && r2.dmgDealt > secondTargetMaxHP * 0.22 && Math.random() < 0.40) {
      await sleep(600);
      if(arena){const a=document.createElement('div');a.className='skill-announce';a.innerHTML='💥反击!';a.style.color='#ff55aa';a.style.textShadow='0 0 20px rgba(255,100,200,0.9),0 0 60px rgba(200,50,150,0.6),2px 2px 0 #550033';arena.appendChild(a);setTimeout(()=>a.remove(),1200);}
      await sleep(400);
...
async function runClassPKTransformSequence(student1, student2, pet1, pet2, robot1, robot2, pet1Img, pet2Img) {
  ...
  img1.innerHTML = `
    <div style="position:relative;width:100%;height:100%;display:flex;align-items:center;justify-content:center;">
      <img src="${robot1||''}" style="max-width:100%;max-height:100%;object-fit:contain;opacity:0;animation:monsterReveal 0.8s ease-out forwards;filter:drop-shadow(0 10px 30px rgba(0,0,0,0.9));" onerror="this.style.display='none';this.parentNode.innerHTML='<span style=\\'font-size:120px;opacity:0;animation:monsterReveal 0.8s ease-out forwards;display:inline-block;filter:drop-shadow(0 0 20px rgba(100,150,255,0.6);\\'>🤖</span>'+this.parentNode.querySelector('.cockpit-pet')?.outerHTML;">
      ${cockpitHtml1}
    </div>
  `;
  ...
  img2.innerHTML = `
    <div style="position:relative;width:100%;height:100%;display:flex;align-items:center;justify-content:center;">
      <img src="${robot2||''}" style="max-width:100%;max-height:100%;object-fit:contain;opacity:0;animation:monsterRevealRight 0.8s ease-out forwards;filter:drop-shadow(0 10px 30px rgba(0,0,0,0.9));" onerror="this.style.display='none';this.parentNode.innerHTML='<span style=\\'font-size:120px;opacity:0;animation:monsterRevealRight 0.8s ease-out forwards;display:inline-block;filter:drop-shadow(0 0 20px rgba(255,100,150,0.6);\\'>🤖</span>'+this.parentNode.querySelector('.cockpit-pet')?.outerHTML;">
      ${cockpitHtml2}
    </div>
  `;
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      // 左侧：飞向机器人胸部偏右位置（约38%高度，58%宽度）
      const targetSize = 45; // 最终小体型
      ...
      // 右侧：飞向机器人胸部偏左位置（约38%高度，42%宽度）
      const targetX2 = img2Rect.left + img2Rect.width * 0.46 - targetSize/2;
      const targetY2 = img2Rect.top + img2Rect.height * 0.38 - targetSize/2;
      petFly2.style.width = targetSize + 'px';
      petFly2.style.height = targetSize + 'px';
...
function getPetImageSrc(petName, level) {
  ...
  if(!cfg) return '';
  return `images/${cfg.id}/${level}.png`;
}
...
async function startJianghuBattle(overlay, boss, student, pet, investCoins, petVisual) {
  ...
  scene.innerHTML = `
    ...
    <div class="jh-battle-boss" id="jhBBoss" style="width:min(600px,50vw);height:min(600px,60vh)">${jhGenBossSVG(boss)}</div>
