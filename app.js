...
function deleteClass(id){if(!confirm('确定删除该班级？删除后可在"已删除班级"中恢复'))return;var cls=classesData.find(function(c){return c.id===id||c.id==id;});if(cls){var snapshot={id:cls.id,name:cls.name,students:JSON.parse(JSON.stringify(cls.students)),deletedAt:new Date().toISOString()};deletedClasses.push(snapshot);if(deletedClasses.length>20)deletedClasses.shift();saveDeletedClasses();}classesData=classesData.filter(function(c){return c.id!==id&&c.id!=id;});if(currentClassId===id||currentClassId==id)currentClassId=classesData[0]?.id||null;if(typeof customActions!=='undefined'){customActions=customActions.filter(function(a){return a.class_id!=id;});}saveClassData();renderClassList();scheduleAllRenders();showNotification('班级已删除','可在"已删除班级"中恢复','info');}
