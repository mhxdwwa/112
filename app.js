...
function deleteClass(id){if(!confirm('确定删除该班级？删除后可在"已删除班级"中恢复'))return;var cls=classesData.find(function(c){return c.id===id||c.id==id;});var _className=cls?cls.name:'';if(cls){var snapshot={id:cls.id,name:cls.name,students:JSON.parse(JSON.stringify(cls.students)),deletedAt:new Date().toISOString()};deletedClasses.push(snapshot);if(deletedClasses.length>20)deletedClasses.shift();saveDeletedClasses();}classesData=classesData.filter(function(c){return c.id!==id&&c.id!=id;});if(currentClassId===id||currentClassId==id)currentClassId=classesData[0]?.id||null;if(typeof customActions!=='undefined'){customActions=customActions.filter(function(a){return a.class_id!=id;});}saveClassData();renderClassList();scheduleAllRenders();showNotification('班级已删除','可在"已删除班级"中恢复','info');if(typeof _supabaseDeleteClass==='function'){_supabaseDeleteClass(id,_className).catch(function(e){console.warn('[DAL] deleteClass Supabase cleanup error:',e);});}}
...
function deleteStudentById(id){if(confirm('删除学生')){const cur=classesData.find(c=>c.id===currentClassId);const _deletedStu=cur.students.find(s=>s.id.toString()===id.toString());cur.students=cur.students.filter(s=>s.id.toString()!==id.toString());saveClassData();closeModal();showStudentListModal();scheduleAllRenders();if(currentModalStudentId&&currentModalStudentId===id)closeModal();if(typeof db!=='undefined'&&db&&_deletedStu&&_deletedStu.id>0){(async function(){try{var stuId=parseInt(_deletedStu.id);if(isNaN(stuId)||stuId<=0)return;var petDel=await db.from('pets').delete().in('student_id',[stuId]);if(petDel.error)console.warn('[DAL] deleteStudent pets error:',petDel.error);var stuDel=await db.from('students').delete().eq('id',stuId);if(stuDel.error)console.warn('[DAL] deleteStudent error:',stuDel.error);else console.log('[DAL] Deleted student',stuId,'from Supabase');}catch(e){console.warn('[DAL] deleteStudent Supabase cleanup error:',e);}})();}}}
function clearAllStudents(){if(confirm('清空所有学生？')){const cur=classesData.find(c=>c.id===currentClassId);const _deletedIds=cur.students.filter(s=>s.id>0&&s.id===Math.floor(s.id)).map(s=>parseInt(s.id));cur.students=[];saveClassData();closeModal();scheduleAllRenders();if(currentModalStudentId)closeModal();if(typeof db!=='undefined'&&db&&_deletedIds.length>0){(async function(){try{var petDel=await db.from('pets').delete().in('student_id',_deletedIds);if(petDel.error)console.warn('[DAL] clearAllStudents pets error:',petDel.error);var stuDel=await db.from('students').delete().in('id',_deletedIds);if(stuDel.error)console.warn('[DAL] clearAllStudents error:',stuDel.error);else console.log('[DAL] Cleared',_deletedIds.length,'students from Supabase');}catch(e){console.warn('[DAL] clearAllStudents Supabase cleanup error:',e);}})();}}}
