...
function showUpgradeEffect(petName, level, petConfigId, petNickname, oldLevel, studentName) {
  ...
  const oldImgSrc = `https://fastly.jsdelivr.net/gh/mhxdwwa/112@master/images/${petConfigId}/${oldLevel}.webp`;
  const newImgSrc = `https://fastly.jsdelivr.net/gh/mhxdwwa/112@master/images/${petConfigId}/${level}.webp`;
...
function autoDiscoverPets() {
  ...
  function checkNext(id) {
    ...
    // 尝试加载该宠物的 1 级图片
    img.src = `https://fastly.jsdelivr.net/gh/mhxdwwa/112@master/images/${id}/1.webp`;
  }
...
function getPetImage(petName, level=1) { const cfg = PET_CONFIG[petName]; if (!cfg) return '<span>🐾</span>'; return `<img src="https://fastly.jsdelivr.net/gh/mhxdwwa/112@master/images/${cfg.id}/${level}.webp" alt="${petName}" loading="lazy" decoding="async" style="max-width:100%; max-height:100%; object-fit: contain;" onerror="this.onerror=null; this.parentNode.innerHTML='<span style=\'font-size:48px;\'>${cfg.emoji}</span>';">`; }
function getEggImage() { return `<img src="https://fastly.jsdelivr.net/gh/mhxdwwa/112@master/images/蛋.webp" alt="宠物蛋" class="egg-img" loading="lazy" decoding="async" style="max-width:100%; max-height:100%; object-fit: contain;" onerror="this.onerror=null; this.parentNode.innerHTML='<span style=\'font-size:60px;\'>🥚</span>';">`; }
...
function probePKMonsterImages() {
  return new Promise((resolve) => {
    ...
    for(let i = 1; i <= 50; i++) {
      tryLeft(`https://fastly.jsdelivr.net/gh/mhxdwwa/112@master/战斗兽宠文件夹/${i}.webp`);
    }
    // 右侧池：所有以0开头的数字图片（01~09, 010~019, 020~024等）
    for(let i = 1; i <= 9; i++) {
      tryRight(`https://fastly.jsdelivr.net/gh/mhxdwwa/112@master/战斗兽宠文件夹/0${i}.webp`);
    }
    for(let i = 10; i <= 50; i++) {
      const padded3 = String(i).padStart(3, '0');
      tryRight(`https://fastly.jsdelivr.net/gh/mhxdwwa/112@master/战斗兽宠文件夹/${padded3}.webp`);
    }
...
function probeJhBossImages() {
  return new Promise((resolve) => {
    ...
    jhBossNames.forEach(name => tryPath(`https://fastly.jsdelivr.net/gh/mhxdwwa/112@master/战斗兽宠文件夹/${name}.webp`));
    setTimeout(finish, 5000);
  });
}
...
function probeClassPKRobotImages() {
  return new Promise((resolve) => {
    ...
    for(let i = 1; i <= 50; i++) {
      pending++;
      const img = new Image();
      const path = `https://fastly.jsdelivr.net/gh/mhxdwwa/112@master/战斗机器人/${i}.webp`;
...
function getPetImageSrc(petName, level) {
  ...
  if(!cfg) return '';
  return `https://fastly.jsdelivr.net/gh/mhxdwwa/112@master/images/${cfg.id}/${level}.webp`;
}
...
function jhGenBossSVG(boss) {
  return `<img src="https://fastly.jsdelivr.net/gh/mhxdwwa/112@master/战斗兽宠文件夹/${esc(boss.name)}.webp" alt="${esc(boss.name)}" style="width:100%;height:100%;object-fit:contain;filter:drop-shadow(0 0 20px ${boss.accent});"/>`;
}
