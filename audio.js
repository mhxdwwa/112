...
(function(){
  ...
  // Wrap switchPage after all scripts have loaded
  // Use setTimeout to defer until after app.js has executed
  setTimeout(function() {
    const origSP = window.switchPage;
    if (typeof origSP === 'function') {
      window.switchPage = function(pid) {
        origSP(pid);
        if (bgPlaying) startMusic(pid); else curPage = pid;
      };
    } else {
      console.warn('[audio.js] switchPage not found, BGM switching disabled');
    }
  }, 0);
