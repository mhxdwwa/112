...
export const onRequestGet = async ({ request, env, params }) => {
  ...
  const [classR, studentsR, petsR] = await Promise.all([
    ...
    sbSelect(env, 'pets', 'id,student_id,name,nickname,level,growth,coins,is_active,is_dead,last_feed_date,last_play_date,today_feed_count,today_play_count,penalty_streak', `student_id=in.(SELECT id FROM students WHERE class_id=${classId})`),
  ]);
