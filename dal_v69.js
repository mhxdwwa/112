  ...
  return syncFn().then(function(result) {
    ...
    if (currentUser && currentUser.type === 'student') {
      ...
      if (shouldUpdateBase) {
        ...
        if (myStu) {
          // v95: REMOVED `_myBaseCoins = myStu.coins` — this was a race condition.
          // _myBaseCoins is already correctly set to `finalCoins` inside _syncStudentToSupabase()
          // (line ~2423). Overwriting it here with myStu.coins (which may have changed due to
          // additional purchases during the async gap) corrupts the delta calculation,
          // causing the last coin deduction to be lost on consecutive purchases.
...
function _setupRealtimeSubscriptions() {
  ...
  try {
    ...
    // This catches the case where channels subscribe but never deliver events
    // (e.g., tables not in supabase_realtime publication, RLS blocking cross-user events).
    _realtimeLastEventTime = Date.now(); // Initialize liveness tracker
...
// If Realtime is truly working, the safety-net just confirms data is fresh (cheap no-op).
// If Realtime is silently broken (subscribed but no events), this catches it within 15s.
function _startSafetyNetPolling() {
  if (_safetyNetTimer) return; // Already running
...
function _safetyNetTick() {
  // v95: Liveness check — if Realtime claims to be active but no events have arrived
  // for _REALTIME_LIVENESS_TIMEOUT ms, mark it as dead and force a refresh.
  ...
  if (_realtimeActive && timeSinceLastEvent > _REALTIME_LIVENESS_TIMEOUT) {
    ...
    _startFallbackPolling();
    // Force an immediate refresh since we may have missed many changes
    _refreshFromSupabase();
    return;
  }
  // Even if Realtime is working, do a lightweight sync check to catch any missed events.
  // This is a safety net — if Realtime missed an event, this poll catches it within 15s.
...
function _cleanupRealtime() {
  ...
  _realtimeActive = false;
  _stopFallbackPolling();
  _stopSafetyNetPolling(); // v95: Also stop safety-net
}
