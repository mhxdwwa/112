-- 修复：删除所有 buy_item 函数重载，只保留 integer 版本
-- 在 Supabase SQL Editor 中执行此脚本

-- 1. 删除 bigint 版本（如果存在）
DROP FUNCTION IF EXISTS buy_item(bigint, bigint, text, integer, text);

-- 2. 删除 integer 版本（如果存在），然后重新创建
DROP FUNCTION IF EXISTS buy_item(integer, integer, text, integer, text);

-- 3. 重新创建唯一的 integer 版本
