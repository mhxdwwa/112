...
function _saveSnackConfigToSupabase(config) {
  ...
  try {
    fetch('/api/snack/config', {
      ...
      .then(data => {
        if (data.ok) {
          console.log('[v199] Snack config saved to Supabase');
        } else {
          console.warn('[v199] Snack config save failed:', data.error);
        }
      })
      .catch(err => {
        console.warn('[v199] Snack config save error:', err);
      });
  } catch(e) {
    console.warn('[v199] Snack config save exception:', e);
  }
}
...
function resetToDefaultSnacks() {
  ...
  if (typeof currentUser !== 'undefined' && currentUser && currentUser.type === 'teacher') {
    fetch('/api/snack/config', {
      ...
      console.warn('[v199] Reset snack config failed:', err);
    });
  }
